...
RUN_NAME="${RUN_NAME:-robotwin_place_can_targeted_oval_pairs_20260906}"
...
START_CANDIDATE_SEED="${START_CANDIDATE_SEED:-4800400}"
MAX_CANDIDATE_SEED="${MAX_CANDIDATE_SEED:-4800479}"
...
for value in "${TARGET_FAILURES}" "${INITIAL_CANDIDATES}" \
  ...
  [[ "${value}" =~ ^[1-9][0-9]*$ ]] || {
    printf '[targeted-oval] invalid positive integer: %s\n' "${value}" >&2
    exit 1
  }
done
(( INITIAL_CANDIDATES <= MAX_CANDIDATES )) || {
  printf '[targeted-oval] initial candidates exceed maximum\n' >&2
  exit 1
}
...
while true; do
  printf '[targeted-oval] collect basket_id=0 candidates=%s target_failures=%s\n' \
  ...
  env RUN_NAME="${RUN_NAME}" TASKS="${TASK}" \
    ...
    REQUIRE_BASKET_ID=0 REQUIRE_ARM_PATTERN=alternating-left-first \
  ...
  printf '[targeted-oval] strict natural failures=%s/%s\n' \
    "${failure_count}" "${candidates}"
  if (( failure_count >= TARGET_FAILURES )); then
    break
  fi
  if (( candidates >= MAX_CANDIDATES )); then
    printf '[targeted-oval] insufficient natural failures at maximum candidates\n' >&2
    exit 1
  fi
  candidates="$((candidates + CANDIDATE_INCREMENT))"
  (( candidates > MAX_CANDIDATES )) && candidates="${MAX_CANDIDATES}"
done
...
printf '[targeted-oval] complete: %s\n' "${SELECTION_AUDIT}"
