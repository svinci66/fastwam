...
# Resumable stage-2 pipeline:
# 10 tasks x 30 train states x (policy + two noise levels), replay build,
# structural/seed audit, and matched residual-IQL training.
...
read -r -a NOISE_STD_ARRAY <<< "${NOISE_STDS}"
if ((${#TASK_ID_ARRAY[@]} == 0 || ${#STATE_INDEX_ARRAY[@]} == 0 || ${#NOISE_STD_ARRAY[@]} == 0)); then
  printf 'Task IDs, state indices, and noise stds must be non-empty.\n' >&2
  exit 2
fi
...
verify_raw_task_mode() {
  ...
  if ((metadata_count <= 0 || metadata_count != arrays_count || metadata_count < expected)); then
    printf 'Invalid raw transitions under %s: metadata=%s arrays=%s expected_at_least=%s\n' \
      "${root}" "${metadata_count}" "${arrays_count}" "${expected}" >&2
    exit 1
  fi
}
noise_tag() {
  "${PYTHON_BIN}" -c "print(f'{float(\"$1\"):.6f}'.replace('.', 'p'))"
}
...
printf '%s\n' \
  ...
  "independent_episode_seeds=true" \
  "seed=${SEED}" \
  "checkpoint=${CHECKPOINT}" \
...
for task_id in "${TASK_ID_ARRAY[@]}"; do
  behavior_names=("policy")
  behavior_modes=("policy")
  behavior_stds=("0.0")
  behavior_streams=("0")
  stream=1
  for noise_std in "${NOISE_STD_ARRAY[@]}"; do
    tag="$(noise_tag "${noise_std}")"
    behavior_names+=("noise_${tag}")
    behavior_modes+=("noise")
    behavior_stds+=("${noise_std}")
    behavior_streams+=("${stream}")
    stream=$((stream + 1))
  done

  for behavior_index in "${!behavior_names[@]}"; do
    behavior="${behavior_names[behavior_index]}"
    mode="${behavior_modes[behavior_index]}"
    noise_std="${behavior_stds[behavior_index]}"
    ...
    if ! stage_done "${stage}"; then
      mode_args=(
        ...
        "EVALUATION.action_seed_stream=${action_stream}"
      )
      if [[ "${mode}" == "noise" ]]; then
...
if ! stage_done validate_iql; then
  for variant in no_imagination imagination; do
    ...
    run_logged "validate_iql_${variant}" "${PYTHON_BIN}" scripts/train_libero_residual_iql.py \
      --config "${config}" \
      --replay-dir "${REPLAY_DIR}" \
      --output-dir "${OUTPUT_ROOT}/unused_validate_${variant}" \
      --validate-only
  done
  mark_done validate_iql
fi

if [[ "${SKIP_TRAIN}" != "1" ]] && ! stage_done train_iql_pair; then
  run_logged train_iql_pair bash scripts/run_libero_residual_iql_pair.sh \
    --replay-dir "${REPLAY_DIR}" \
    --output-root "${OUTPUT_ROOT}/iql_pair" \
    --device cuda
  mark_done train_iql_pair
fi
