...
for ((attempt = 1; attempt <= MAX_ONLINE_ATTEMPTS; attempt++)); do
  printf '[video-expert-three-way] online attempt=%d/%d\n' \
    "${attempt}" "${MAX_ONLINE_ATTEMPTS}"
  if run_online_pair; then
    break
  fi
  if (( attempt == MAX_ONLINE_ATTEMPTS )); then
    printf '[video-expert-three-way] exhausted online attempts\n' >&2
    exit 1
  fi
  printf '[video-expert-three-way] online attempt failed; resuming completed tasks\n' >&2
done
