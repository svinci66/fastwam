...
RUN_NAME="${RUN_NAME:-robotwin_imagination_reward_triplet_smoke_20260825}"
...
branch_complete() {
  local tag="$1" mode_tag="$2" root log_file video
  ...
  [[ -n "${log_file}" ]] && rg -q 'Success rate:' "${log_file}" \
    && [[ -s "${video}" ]] \
    && [[ -f "${root}/imagination_transitions/${TASK}/${mode_tag}/episode_0000/replan_$(printf '%04d' "${INTERVENTION_REPLAN}")/metadata.json" ]]
}

run_branch() {
  local tag="$1" mode="$2" noise_std="$3" mode_tag="$4"
  if branch_complete "${tag}" "${mode_tag}"; then
    printf '[triplet-smoke] skip complete branch=%s\n' "${tag}"
    return
  fi
  printf '[triplet-smoke] start branch=%s mode=%s replan=%s max_abs_delta=%s\n' \
  ...
  timeout --verbose --signal=TERM --kill-after=30s 2400s \
    ...
    EVALUATION.environment_start_seed=4800003 \
  ...
  branch_complete "${tag}" "${mode_tag}" || {
    printf '[triplet-smoke] incomplete branch=%s\n' "${tag}" >&2
    exit 65
  }
  printf '[triplet-smoke] complete branch=%s\n' "${tag}"
}
...
conda run --no-capture-output -n "${CONDA_ENV}" python -u \
  ...
  --clean-root "$(transition_root clean policy)" \
  --corrupt-root "$(transition_root corrupted "controlled_corrupt_$(printf '%.3f' "${MAX_ABS_DELTA}")")" \
  --correct-root "$(transition_root corrected "controlled_correct_$(printf '%.3f' "${MAX_ABS_DELTA}")")" \
  --intervention-replan "${INTERVENTION_REPLAN}" \
  --output-json "${ARTIFACT_DIR}/triplet_audit.json"
...
printf '[triplet-smoke] PASS audit=%s\n' "${ARTIFACT_DIR}/triplet_audit.json"
